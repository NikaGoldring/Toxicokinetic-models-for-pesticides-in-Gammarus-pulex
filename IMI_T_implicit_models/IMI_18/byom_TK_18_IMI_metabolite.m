...
%         9	9	10
%         9	10	10
%         10	10	9]; % each point is a pooled sample of X animals   % OR: 8 * ones(size(DATA{1})-1); 

%% Initial values for the state variables
% Initial states, scenarios in columns, states in rows. First row are the
% 'names' of all scenarios.

X0mat = [17.58   % the scenarios (here nominal concentrations) 
         0
...
par.ke    = [0.098247   1 1e-4 100 1];  % elimination rate constant, d-1
par.ku    = [2.9644     1 1e-4 100 1];  % uptake rate constant, L/kg/d
par.km    = [0.023319   1 1e-4 100 1];  % formation rate of the metabolite
par.kem   = [0.68435    1 1e-4 100 1];  % elimination rate of the metabolite
